...
const Search = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://api.themoviedb.org/3/search/multi?api_key=${process.env.REACT_APP_API_KEY}&query=${params.name}&page=${params.page}`
      )
      .then((response) => {
        setIsShimmer(false);
        ...
        {isShimmer
          ...
          : movies.map((element, index) => (
              <Card
                key={index}
                element={element}
                type={element.media_type === "tv" ? "Tv-series" : "Movie"}
              />
            ))}
      </div>
      <div style={{ display: "flex", justifyContent: "center" }}>
        {!error&&<div className={styles.btnContainer}>
          <button
            ...
            onClick={() => {
              setMovies([]);
              navigate(`/search/${params.name}/${Number(params.page) - 1}`);
            }}
          >
            Back
          </button>
